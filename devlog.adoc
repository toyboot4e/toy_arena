= `toy_arena` devlog
:toy_arena: https://docs.rs/toy_arena/latest/toy_arena/[toy_arena]
...
** Slot-based operations are removed. Instead, `EntryBindMut` type has slot-changing methods such as `remove`.
+
